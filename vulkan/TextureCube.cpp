...
bool TextureCube::Init(const std::shared_ptr<Device>& pDevice, const std::shared_ptr<TextureCube>& pSelf, const GliImageWrapper& gliTexCube, VkFormat format)
{
	...
	uint32_t mipLevels = (uint32_t)gliTexCube.textures[0].levels();
	...
	textureCreateInfo.arrayLayers = (uint32_t)gliTexCube.textures[0].faces();
...
std::shared_ptr<TextureCube> TextureCube::CreateCubeTexture(const std::shared_ptr<Device>& pDevice, std::string path, VkFormat format)
{
...
uint32_t TextureCube::ExecuteCopy(const GliImageWrapper& gliTex, uint32_t layer, const std::shared_ptr<StagingBuffer>& pStagingBuffer, uint32_t offset, const std::shared_ptr<CommandBuffer>& pCmdBuffer)
{

...
bool GlobalDeviceObjects::InitObjects(const std::shared_ptr<Device>& pDevice)
{
	...
	m_pUniformBufferMgr = SharedBufferManager::Create(pDevice, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, UNIFORM_BUFFER_SIZE);
	m_pShaderStorageBufferMgr = SharedBufferManager::Create(pDevice, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, (VkMemoryPropertyFlagBits)(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT), UNIFORM_BUFFER_SIZE);
